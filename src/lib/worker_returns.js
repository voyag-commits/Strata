...
import { exists, fileSlug, isoNow, readJson, relativeOrAbsolute, resultEnvelope, sha256File, timestamp, workspacePath, writeJson } from "./common.js";
...
export function classifyWorkerReturnPacket(root, packetPathInput) {
  const packetPath = relativeOrAbsolute(root, packetPathInput);
  let raw = null;
  let readErrors = [];
  try { raw = readJson(packetPath); } catch (error) { readErrors = [`failed to read JSON packet: ${error instanceof Error ? error.message : String(error)}`]; }
  ...
  if (!packet) {
    ...
    errorDispatch = recordErrorDispatch(root, { assignmentId: raw?.assignment_id || "UNKNOWN_ASSIGNMENT", targetRole: raw?.role || "Sender Session", targetId: raw?.agent_id || "sender_session", summary: "Worker return packet validation failed.", declaredFiles: [path.relative(root, packetPath)] });
    ...
    if (routing !== "OPERATIONAL_REPORT_READY_INVALID_TO_CLASSD") {
      const reportValidation = reportPath ? validateOperationalReportFile(root, { file: reportPath, requireNonEmptySections: true }) : { ok: false, errors: ["report_path missing"] };
      if (!reportPath || !exists(reportPath) || !reportValidation.ok) {
        ...
        routing = "OPERATIONAL_REPORT_READY_INVALID_TO_CLASSD";
        errorDispatch = recordErrorDispatch(root, { assignmentId: packet.assignment_id, targetRole: packet.role, targetId: packet.agent_id, summary: "Operational report validation failed.", declaredFiles: [path.relative(root, packetPath), packet.report_path] });
      } else {
        const l = ensureLayout(root);
