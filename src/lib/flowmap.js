...
export function buildWorkflowFlowmap(root, input = {}) {
  ...
  const steps = [
    ...
    {
      step: 1,
      name: "Adapter gets called",
      call: 'scripts/wsl_tmux/sctl-session-new --assignment-id A_LIVE_001 --role "Reviewer / QC Engineer" --id reviewer_live_001 --session-name STRATA-REVIEW-A-LIVE-001',
    ...
    {
      step: 2,
      name: "Class C team message is created",
      call: "node src/cli.js message send ...",
      result: `${messageFiles.length} matching message file(s) found`,
      diagnosis: messageFiles.length ? "PASS: Class C message exists in context Git for this assignment/target." : "FAIL: no matching Class C message was found.",
    ...
    },
    {
      ...
      diagnosis: exists(packetMd) && packetText.includes("# Class C Team Message") && packetText.includes("# Below is system level full context picture.")
        ? "PASS: canonical Git-backed packet contains Class C message and full context headline."
    ...
    {
      step: 4,
      name: "Injection happens",
      call: `scripts/wsl_tmux/sctl-dispatch-inject --session ${sessionName} --packet ${path.relative(root, packetMd)}`,
      result: exists(packetMd) ? "canonical packet is available for adapter injection" : "canonical packet unavailable",
      diagnosis: "INSPECT: SCTL proves packet/session inputs; live session receipt must be observed through tmux/capture.",
      watch: [pathCheck(packetMd)],
    },
    {
      step: 5,
      name: "Session work is captured",
      call: `scripts/wsl_tmux/sctl-session-capture --session ${sessionName} --out .strata/evidence/session_captures/${sessionName}.txt`,
