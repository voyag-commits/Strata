...
export function registerSession(root, input = {}) {
  ...
  const session = {
    contract_id: "strata.class_c.active_session.v2_simplified_runtime",
    role,
    id,
    session_name: input.sessionName || input.session_name || id,
    ...
    delivery_mode: "paste_only_deterministic_envelope",
...
export function retireSession(root, input = {}) {
  ...
  const next = sessions.map((s) => (s.id === id && s.assignment_id === assignmentId ? { ...s, status: "retired", retired_at: isoNow(), retire_reason: input.reason || "session lifecycle closed" } : s));
  const file = activeSessionsPath(root);
  writeText(file, `${JSON.stringify({ contract_id: "strata.class_c.active_sessions.v2_simplified_runtime", sessions: next, updated_at: isoNow() }, null, 2)}\n`);
  const lifecyclePath = writeText(path.join(l.sessionLifecycle, `${fileSlug(`${assignmentId}_${id}_retired_${timestamp()}`)}.json`), `${JSON.stringify({ contract_id: "strata.session_lifecycle_event.v1", event: "retired", assignment_id: assignmentId, id, reason: input.reason || null, created_at: isoNow() }, null, 2)}\n`);
  const telemetry = recordTelemetry(root, "session.retire.completed", { result: "ok", assignment_id: assignmentId, target_id: id });
  const git = gitCommitContext(l.context, `session retire ${assignmentId} ${id}`, { paths: [contextRel(root, file), contextRel(root, lifecyclePath), contextRel(root, telemetry.path)] });
  return resultEnvelope("sctl.sessions.retire.v1", true, { id, assignment_id: assignmentId, file, lifecycle_path: lifecyclePath, git }, [], [file, lifecyclePath, telemetry.path]);
}
