<#
.SYNOPSIS
  Install the Strata SCTL appliance WSL distro (arm64) from MyDistro-arm64.tar.
.DESCRIPTION
  Imports the prebuilt arm64 rootfs as a new WSL2 distro named "SCTL-A004-arm64",
  creates the strata user default, and runs first-boot locale generation.
  For ARM64 Windows hosts (e.g. Snapdragon X, Surface Pro X).
.PARAMETER DistroName
  Name for the imported distro. Default: SCTL-A004-arm64
.PARAMETER TarPath
  Path to MyDistro-arm64.tar. Default: same directory as this script.
.PARAMETER InstallDir
  Where to store the distro VHDX. Default: $env:LOCALAPPDATA\WSL\SCTL-A004-arm64
.EXAMPLE
  .\install-arm64.ps1
  .\install-arm64.ps1 -DistroName MyStrata -TarPath C:\tars\MyDistro-arm64.tar
#>
param(
  [string]$DistroName = "SCTL-A004-arm64",
  [string]$TarPath = "",
  [string]$InstallDir = ""
)
...
if (-not $TarPath) {
  $TarPath = Join-Path $PSScriptRoot "MyDistro-arm64.tar"
}
if (-not $InstallDir) {
...
if ($arch -ne "Arm64" -and $arch -ne "arm64") {
  Write-Warning "This arm64 build is for ARM64 Windows. Detected arch: $arch"
  Write-Warning "If you are on x64 Windows, use install-amd64.ps1 instead."
}
