...
export function loadConfigFile(file) {
  ...
  const env = {};
  for (const line of readText(resolved).split(/\r?\n/)) {
...
export function applyConfig(flags) {
  const configFile = optionalFlag(flags, "config");
  if (!configFile) return;
  const env = loadConfigFile(configFile);
...
export function safeSegment(value) {
  ...
  if (cleaned === "." || cleaned === ".." || /^\.+$/.test(cleaned)) throw new Error("safe segment cannot be a dot-only path segment");
  return cleaned;
}
