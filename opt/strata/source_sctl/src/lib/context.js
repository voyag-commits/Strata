...
export const CLASS_B_LATEST_DISPATCH_COUNT = 2;
export const COORDINATOR_RETIRE_CYCLE_COUNT = 4;
...
export function incrementClassBState(root) {
  ...
  return writeContextState(root, {
    ...
    refresh_required: Boolean(state.refresh_required),
    refresh_reason: state.refresh_reason || null,
  });
}
...
export function contextFreshness(root, input = {}) {
  ...
  const loadedA = loadedClassA === undefined || loadedClassA === null || loadedClassA === "" ? 0 : Number(loadedClassA);
  ...
  if (classADelta > 0) {
    ...
    action = "latest_class_b_context_export";
    exportMode = "class_b_latest";
  }
  return resultEnvelope("sctl.context.freshness.v1", true, {
    ...
    class_b_latest_count: CLASS_B_LATEST_DISPATCH_COUNT,
    coordinator_retire_cycle_count: COORDINATOR_RETIRE_CYCLE_COUNT,
    cycle_delta_reports: CLASS_B_LATEST_DISPATCH_COUNT,
    math: `${currentB}-${loaded}=${classBDelta}`,
    policy: "Class A changes require retire and full context. Missing loaded_class_a_revision is treated as unknown stale revision 0. Class B dispatch uses latest 2 report records by default; coordinator retire/recreate is controlled by cycle count 4, not by Class B accumulation.",
    state,
  }, [], []);
}
