...
## ADR 06/26 update note

ADR 06/26 updates the runtime lifecycle portions of this flow. Canonical live acceptance now uses the runtime delegate `session-create` verb to launch or resolve runtime sessions and `dispatch-deliver` to deliver through the delegate registry. Normal Flowmap 02 marks role sessions released or superseded logically and does not call `session-terminate`; runtime cleanup is explicit operator/diagnostic action only.
...
-> logically release disposable sessions; runtime sessions remain alive
...
**Call:** `strata-runtime-edge delegate session-register --assignment-id "$ASSIGNMENT_ID" --role "Trunk Coordinator" --id "$TRUNK_COORDINATOR_ID" --session-name "$TRUNK_COORDINATOR_SESSION" --session-mode long_running`

**Method:** `delegate session-register -> direct runtime delegate contract call session launch OR strata-fleet-launch -> sessions register -> registerSession() -> recordTelemetry() -> gitCommitContext()`

**Watch:** `.strata/context/C/sessions/active_sessions.json`; `.strata/context/C/sessions/lifecycle/`; delegate runtime sessions remain alive.; `.strata/context/D_trace/telemetry/`
...
**Call:** `strata-runtime-edge delegate session-register --assignment-id "$ASSIGNMENT_ID" --role "Change Author" --id "$CHANGE_AUTHOR_ID" --session-name "$CHANGE_AUTHOR_SESSION" --session-mode disposable`
...
**Call:** `scripts/wsl_tmux/sctl-dispatch-render --assignment-id "$ASSIGNMENT_ID" --nonce "$NONCE_AUTHOR" --target-role "Change Author" --target-id "$CHANGE_AUTHOR_ID" --related-class-b ".strata/context/B/<coordinator_work_order>.md" --summary "Flowmap 02 Change Author dispatch"`
...
**Call:** `strata-runtime-edge delegate dispatch-deliver --session "$CHANGE_AUTHOR_SESSION" --packet ".strata/context/D_trace/dispatch_packets/$ASSIGNMENT_ID/$CHANGE_AUTHOR_ID/$NONCE_AUTHOR/dispatch_packet.md"`

**Method:** `delegate dispatch-deliver -> tmux set-buffer -> tmux paste-buffer -> tmux send-keys Enter`
...
**Call:** `strata-runtime-edge delegate session-register --assignment-id "$ASSIGNMENT_ID" --role "Code Reviewer / QC Engineer" --id "$REVIEWER_ID" --session-name "$REVIEWER_SESSION" --session-mode disposable`
...
**Call:** `scripts/wsl_tmux/sctl-dispatch-render --assignment-id "$ASSIGNMENT_ID" --nonce "$NONCE_REVIEW" --target-role "Code Reviewer / QC Engineer" --target-id "$REVIEWER_ID" --related-class-b ".strata/context/B/<author_report>.md" --summary "Flowmap 02 review dispatch"`
...
**Call:** `strata-runtime-edge delegate dispatch-deliver --session "$REVIEWER_SESSION" --packet ".strata/context/D_trace/dispatch_packets/$ASSIGNMENT_ID/$REVIEWER_ID/$NONCE_REVIEW/dispatch_packet.md"`
...
**Method:** `sendTeamMessage() -> recordDispatch() -> renderDispatchPacket() -> exportMarkdown() -> delegate dispatch-deliver`
...
**Call:** `$SCTL sessions release --id "$CHANGE_AUTHOR_ID" --status released --reason "deny_count_gt_1"`, then create a fresh Author logical session through delegate `session-create` and dispatch full context with denial history.

**Method:** `sessions release -> releaseSession() -> delegate session-create -> registerSession() with runtime identity -> sctl-dispatch-render -> recordDispatch() -> renderDispatchPacket() -> exportMarkdown() -> delegate dispatch-deliver`

**Watch:** old author logical session released with runtime left alive; new author launched/resolved through delegate; new dispatch includes denial history.
...
### 18. Disposable sessions are logically released

**Call:** `$SCTL sessions release --id "$CHANGE_AUTHOR_ID" --status released --reason "flowmap_02_cycle_complete"`; repeat for reviewer. Do not call delegate `session-terminate` in normal Flowmap 02.

**Method:** `sessions release -> releaseSession() -> recordTelemetry() -> gitCommitContext()`

**Watch:** `.strata/context/C/sessions/active_sessions.json`; `.strata/context/C/sessions/lifecycle/`; delegate runtime sessions remain alive.
...
**Policy:** Class A change means full context and logical session refresh; runtime sessions are not killed by normal Flowmap 02. Class B refresh uses Class A plus the latest 2 Class B reports by default. Coordinator recreation is driven by completed coordinator-author-reviewer cycle count, with recreation after 4 completed cycles.

**Optional export:** `$SCTL context export-markdown --include-classes A,B --class-b-latest 2`
