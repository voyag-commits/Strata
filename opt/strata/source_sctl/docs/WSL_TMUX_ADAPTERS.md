...
export SCTL_RUNTIME_DELEGATE_ROOT="/path/to/strata_runtime_edge_launcher_delegate_component2_3_v0_9"
```

`SCTL_RUNTIME_DELEGATE_ROOT` is optional. When it is missing, the adapters either use direct tmux commands or register SCTL metadata only.

## Scripts

| Script | Purpose |
|---|---|
| `delegate session-register` | Optionally calls runtime-edge or `strata-fleet-launch`, then registers SCTL session metadata. |
| `sctl-dispatch-render` | Calls `sctl dispatch record` and produces the deterministic dispatch envelope. |
| `delegate dispatch-deliver` | Opens a visible Windows Terminal tab attached to the target tmux session, then pastes `dispatch_packet.md` with `tmux load-buffer`, `tmux paste-buffer`, and optional `tmux send-keys Enter`. |
| `delegate session-capture` | Captures a tmux pane or delegates to runtime-edge capture. |
| `delegate session-terminate` | Terminates runtime/tmux when requested and records session retirement in SCTL. |
