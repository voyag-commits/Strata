...
function textFromUnknown(value: unknown): string {
  ...
  if (isRecord(value)) {
    ...
    if (value.type === "input_image") return "[image input omitted by DeepSeek text bridge]";
    ...
    return content
      .map((part) => {
        ...
        if (isRecord(part)) {
          ...
          if (part.type === "input_image") return "[image input omitted by DeepSeek text bridge]";
...
function appendMessage(messages: ChatMessage[], role: Role, content: string | null, extra: Partial<ChatMessage> = {}): void {
  if (role !== "assistant" && role !== "tool" && (!content || content.length === 0)) return;
  messages.push({ role, content, ...extra });
}

function reasoningText(item: Record<string, unknown>): string {
  const summary = item.summary;
  if (Array.isArray(summary)) return summary.map((part) => textFromUnknown(part)).filter(Boolean).join("\n");
  if (typeof summary === "string") return summary;
  const content = item.content;
  if (Array.isArray(content)) return content.map((part) => textFromUnknown(part)).filter(Boolean).join("\n");
  if (typeof content === "string") return content;
  return "";
}

function attachReasoning(messages: ChatMessage[], text: string): void {
  if (!text) return;
...
function inputItemToMessages(item: unknown, messages: ChatMessage[]): void {
...
export function responsesInputToMessages(request: ResponsesRequest): ChatMessage[] {
  const messages: ChatMessage[] = [];

  const instructions = contentToText(request.instructions);
  if (instructions) appendMessage(messages, "system", instructions);

  const developer = contentToText(request.developer);
  if (developer) appendMessage(messages, "system", developer);

  const input = request.input;
  if (typeof input === "string") {
    appendMessage(messages, "user", input);
  } else if (Array.isArray(input)) {
    for (const item of input) inputItemToMessages(item, messages);
  } else if (input !== undefined && input !== null) {
    inputItemToMessages(input, messages);
  }

  if (messages.length === 0) appendMessage(messages, "user", "");
  return coalesceAssistantToolCalls(messages);
}

function isToolCallOnlyAssistant(message: ChatMessage): boolean {
  return message.role === "assistant" && Array.isArray(message.tool_calls) && message.tool_calls.length > 0 && !message.content;
}

function coalesceAssistantToolCalls(messages: ChatMessage[]): ChatMessage[] {
...
function toolFromResponsesTool(tool: unknown): ChatTool | null {
...
export function responsesToolsToChatTools(tools: unknown[] | undefined): ChatTool[] | undefined {
...
function mapReasoning(request: ResponsesRequest, config: BridgeConfig): DeepSeekChatRequest["thinking"] | undefined {
  ...
  if (!config.enableThinking) return { type: "disabled" };
  ...
  let reasoning_effort: "high" | "max" | undefined = config.reasoningEffort;
  if (!reasoning_effort) {
    const requested = request.reasoning?.effort;
    if (requested === "high") reasoning_effort = "high";
    else if (requested === "xhigh" || requested === "max") reasoning_effort = "max";
  }
  const thinking: NonNullable<DeepSeekChatRequest["thinking"]> = { type: "enabled" };
  if (reasoning_effort) thinking.reasoning_effort = reasoning_effort;
  if (config.thinkingBudget && config.thinkingBudget > 0) thinking.budget_tokens = config.thinkingBudget;
  return thinking;
}

export function buildDeepSeekRequest(request: ResponsesRequest, config: BridgeConfig): DeepSeekChatRequest {
  ...
  if (typeof maxTokens === "number") chatRequest.max_tokens = maxTokens;
  ...
  if (typeof request.stop === "string" || Array.isArray(request.stop)) chatRequest.stop = request.stop;
  ...
  if (thinking) chatRequest.thinking = thinking;

  return chatRequest;
}

function usageFromDeepSeek(usage: DeepSeekChatResponse["usage"]): unknown {
...
export function deepSeekChatToResponses(chat: DeepSeekChatResponse, request: ResponsesRequest): ResponseObject {
  ...
  if (message.reasoning_content) metadata.deepseek_reasoning_discarded = true;
  if (choice?.finish_reason) metadata.deepseek_finish_reason = choice.finish_reason;
...
export function deepSeekErrorToOpenAI(status: number, body: unknown): OpenAIErrorBody {
  let message = `DeepSeek upstream error (${status})`;
  let code: string | number | null = status;
  if (isRecord(body)) {
    if (isRecord(body.error)) {
      message = textFromUnknown(body.error.message) || message;
      code = (body.error.code as string | number | undefined) ?? code;
    } else if (typeof body.message === "string") {
      message = body.message;
      code = (body.code as string | number | undefined) ?? code;
    }
