...
function redactConfiguredSecrets(value: unknown, config: BridgeConfig): unknown {
  ...
  const secrets = [config.deepSeekApiKey, config.bridgeAuthKey].filter(
...
function stripConfiguredSecrets(value: unknown, config: BridgeConfig): unknown {
  const secrets = [config.deepSeekApiKey, config.bridgeAuthKey].filter(
    (secret): secret is string => typeof secret === "string" && secret.length > 0,
  );
...
export function writeRawDebugJson(config: BridgeConfig, filePath: string | null, payload: unknown): void {
  if (!filePath) return;
  if (process.env.BRIDGE_RAW_CAPTURE !== "1") return;
