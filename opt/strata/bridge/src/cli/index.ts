...
async function main(): Promise<void> {
  ...
  if (command === "assignment" && subcommand === "run") {
    ...
    console.log(JSON.stringify(result, null, 2));
    if (!result.summary.ok || result.secretScanStatus === "FAIL") process.exitCode = 1;
    return;
  }
  ...
  if (command === "result" && subcommand === "show") {
    if (typeof flags.run !== "string") throw new Error("Missing --run.");
    const run = findRun(context, flags.run);
    if (!run?.resultPath) throw new Error(`No result found for ${flags.run}.`);
  ...
  if (command === "trace" && subcommand === "show") {
    if (typeof flags.run !== "string") throw new Error("Missing --run.");
    const run = findRun(context, flags.run);
    if (!run?.tracePath) throw new Error(`No trace found for ${flags.run}.`);
