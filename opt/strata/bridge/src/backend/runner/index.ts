...
import { readJson, type CommandContext } from "../common.js";
import { loadRuntimeConfig } from "../config/index.js";
import { runAssignmentAppServer, type AppServerRunSummary } from "../appserver/index.js";
...
export interface RunnerResult {
  summary: AppServerRunSummary;
  resultPath: string;
  tracePath: string;
  secretScanReport: string;
  secretScanStatus: "PASS" | "FAIL";
}

export async function runAssignment(context: CommandContext, assignmentPath: string, secretFile?: string): Promise<RunnerResult> {
  const assignment = readJson<Record<string, unknown>>(path.resolve(assignmentPath));
