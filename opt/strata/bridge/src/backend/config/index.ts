...
function readEvidenceCodexPath(projectRoot: string): string | null {
  const evidencePath = path.join(projectRoot, "m0_cli_appserver_validation_evidence", "01_codex_cli_path.txt");
  if (!fileExists(evidencePath)) return null;
...
export function loadRuntimeConfig(projectRoot: string, overrides?: Partial<RuntimeConfig>): RuntimeConfig {
