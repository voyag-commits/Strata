...
export interface RunCapture {
  ...
  finalMessage: string;
  resultState: "PASS" | "PARTIAL" | "FAIL" | "BLOCKED";
  capturedAt: string;
}
...
export function captureRunResult(context: CommandContext, summaryPath: string): RunCapture {
  ...
  const assignment = readJson<Record<string, unknown>>(assignmentPath);
  ...
  const completionPayload = summary.completionPayload as Record<string, any> | null | undefined;
  const errorMessage =
    typeof completionPayload?.error?.message === "string"
  ...
  const completionStatus = (summary.completionStatus as string | null) ?? null;
  const resultState: RunCapture["resultState"] =
    completionStatus === "completed" && finalMessage
      ? "PASS"
...
export function writeClassBResult(context: CommandContext, capture: RunCapture): ResultArtifact {
...
export function writeClassDTrace(context: CommandContext, capture: RunCapture): TraceArtifact {
  const layout = ensureContextLayout(context.workspaceRoot);
