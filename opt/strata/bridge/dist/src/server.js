...
function responseSkeleton(request, model) {
    return {
        id: makeId("resp"),
        object: "response",
...
async function callDeepSeek(fetchFn, config, payload) {
...
async function* readUpstreamSse(response) {
    if (!response.body)
        return;
...
export async function* streamDeepSeekAsResponses(upstream, request, model) {
    ...
    const ensureTool = function* (index) {
    ...
    for await (const chunk of readUpstreamSse(upstream)) {
        ...
        if (typeof delta.content === "string" && delta.content.length > 0) {
            for (const event of ensureMessage())
                yield event;
            text += delta.content;
        ...
        if (Array.isArray(delta.tool_calls)) {
            for (const rawTool of delta.tool_calls) {
                ...
                if (typeof rawTool.id === "string")
                    state.callId = rawTool.id;
                const fn = isRecord(rawTool.function) ? rawTool.function : {};
                if (typeof fn.name === "string" && fn.name)
                    state.name = fn.name;
                for (const event of ensureTool(toolIndex))
                    yield event;
...
function appendErrorLog(config, record) {
    if (!config.errorLog)
        return;
    try {
...
async function handleResponses(request, reply, config, fetchFn, enableRequestLogs) {
    ...
    for (let attempt = 1; attempt <= MAX_UPSTREAM_ATTEMPTS; attempt += 1) {
        try {
            ...
            if (attempt < MAX_UPSTREAM_ATTEMPTS) {
                if (enableRequestLogs)
                    safeLog("upstream.retry", { attempt, reason: "network_error" });
        ...
        if (!upstream.ok && RETRYABLE_UPSTREAM_STATUS.has(upstream.status) && attempt < MAX_UPSTREAM_ATTEMPTS) {
            if (enableRequestLogs)
                safeLog("upstream.retry", { attempt, status: upstream.status });
    ...
    if (!upstream.ok) {
        ...
        let errorBody;
        try {
            errorBody = rawErrorText ? JSON.parse(rawErrorText) : undefined;
        }
        catch {
...
export function createApp(options) {
    ...
    app.post("/v1/responses", async (request, reply) => handleResponses(request, reply, config, fetchFn, Boolean(options.enableRequestLogs)));
    app.post("/responses", async (request, reply) => handleResponses(request, reply, config, fetchFn, Boolean(options.enableRequestLogs)));
