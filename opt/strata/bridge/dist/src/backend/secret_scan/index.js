...
import { ensureContextLayout, fileStamp, writeText } from "../common.js";
function defaultTargets(workspaceRoot) {
    const rootFiles = fs
...
export function runSecretScan(context, options) {
    ...
    const hasUnredactedPatternMatch = (content) => {
    ...
    for (const target of targets) {
        for (const filePath of collectFiles(target)) {
            if (options.secretFile && path.resolve(filePath) === path.resolve(options.secretFile))
                continue;
            let content = "";
            try {
    ...
    if (options.secretFile)
        lines.push(`excluded=${path.resolve(options.secretFile)}`);
    if (hitList.length > 0) {
        lines.push("hits:");
        lines.push(...hitList);
    }
