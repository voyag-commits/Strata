...
test("Responses request -> DeepSeek request maps max_output_tokens and sampling params", () => {
    const request = buildDeepSeekRequest({ model: "deepseek-v4-flash", input: "Hi", max_output_tokens: 1000, temperature: 0.2, top_p: 0.9, stream: false }, config);
...
test("explicit thinking only enables high/max profiles", () => {
    const request = buildDeepSeekRequest({ input: "Think deeply.", reasoning: { effort: "high" } }, { ...config, enableThinking: true });
