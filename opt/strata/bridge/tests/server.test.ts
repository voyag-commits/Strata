...
test("/v1/models works with proxy auth", async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.deepEqual(response.json().data.map((model: any) => model.id), ["deepseek-v4-pro", "deepseek-v4-flash"]);
  await app.close();
});
...
test("POST /v1/responses performs non-streaming upstream call and response conversion", async () => {
  ...
  const app = createApp({
    ...
    fetchFn: async (_input, init) => {
      ...
      upstreamAuth = String((init?.headers as any).authorization);
      return new Response(
        JSON.stringify({
          id: "chatcmpl_test",
          created: 100,
...
test("streaming DeepSeek tool-call chunks -> Responses function-call events", async () => {
  ...
  const upstreamStream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      controller.enqueue(
        encoder.encode(
          'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\\"a.ts\\"}"}}]},"index":0,"finish_reason":"tool_calls"}]}\n\n',
        ),
      );
      controller.enqueue(encoder.encode("data: [DONE]\n\n"));
      controller.close();
...
test("debug trace writes redacted request and response payloads", async () => {
  ...
  assert.doesNotMatch(
    combined,
    new RegExp([baseConfig.deepSeekApiKey, baseConfig.bridgeAuthKey!, metadataSecret].map(escapeRegExp).join("|")),
  );
