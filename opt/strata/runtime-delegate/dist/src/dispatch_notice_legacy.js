...
import { ensureDir, fileStamp, iso, layoutFor, readJson, readText, runCommand, safePart, sha256Text, writeJson, writeText } from "./common.js";
export function validateNotice(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        throw new Error("notice must be a JSON object");
    const obj = raw;
    if (obj.contract_id !== "strata.runtime_edge_notice.v1")
        throw new Error("contract_id must be strata.runtime_edge_notice.v1");
    const req = (key) => {
        const value = obj[key];
    ...
    if (!Array.isArray(obj.changed_files) || !obj.changed_files.every((item) => typeof item === "string"))
        throw new Error("changed_files must be string[]");
    if (!Array.isArray(obj.suggested_command_options) || !obj.suggested_command_options.every((item) => typeof item === "string"))
        throw new Error("suggested_command_options must be string[]");
    return {
        ...
        nonce: req("nonce"),
        suggested_command_options: obj.suggested_command_options,
        target_session: nullable("target_session"),
        created_at: typeof obj.created_at === "string" ? obj.created_at : undefined,
    };
}
export function renderBoundedNotice(notice, maxBytes = 3900) {
...
export function injectNotice(ctx, input) {
    ...
    const notice = validateNotice(readJson(noticePath));
    ...
    if (!input.dryRun) {
        ...
        if (!tmuxResults.has_session.ok) {
            ...
            if (capture.ok)
                capturePath = writeText(path.join(evidenceDir, "pane_capture.txt"), capture.stdout);
    ...
    const result = { contract_id: "strata.runtime_edge.inject_notice_result.v1", ok, dry_run: Boolean(input.dryRun), session_name: sessionName, evidence_dir: evidenceDir, notice_path: noticePath, message_path: messagePath, blocked_artifact_path: blockedArtifactPath, tmux_results: tmuxResults, capture_path: capturePath };
