...
import { exists, fileStamp, iso, layoutFor, readJson, runCommand, safePart, writeJson } from "./common.js";
...
function validateMode(value) {
    if (value === "exec" || value === "shell" || value === "windows_shortcut")
        return value;
    throw new Error("mode must be exec, shell, or windows_shortcut");
}
export function validateLauncherDelegateConfig(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        throw new Error("delegate config must be a JSON object");
    const obj = raw;
    if (obj.contract_id !== "strata.runtime_edge.launcher_delegate_config.v1")
        throw new Error("contract_id must be strata.runtime_edge.launcher_delegate_config.v1");
    ...
    if (!launcherCommand)
        throw new Error("launcher_command must be a non-empty string");
    const workingDirectory = obj.working_directory == null ? null : (typeof obj.working_directory === "string" ? obj.working_directory : null);
    if (obj.working_directory != null && workingDirectory == null)
        throw new Error("working_directory must be string or null");
    if (obj.secret_policy !== "externalized_no_secret_material_in_package")
        throw new Error("secret_policy must be externalized_no_secret_material_in_package");
    return {
...
export function defaultConfigPath(ctx) {
    return path.join(layoutFor(ctx.workspaceRoot).config, "launcher_delegate.local.json");
}
export function readLauncherDelegateConfig(ctx, configPath) {
    ...
    return { config: validateLauncherDelegateConfig(readJson(resolvedPath)), path: resolvedPath };
}
export function writeLauncherDelegateTemplate(ctx, outPath) {
...
export function resolveLauncherCommand(ctx, config, extraArgs = []) {
...
export function providerDoctor(ctx, configPath, skipHealthcheck = false) {
...
export function commandExistsForExecMode(command) {
