...
export function tmuxDisplayTarget(target) {
    const format = [
...
export function tmuxLoadAndPaste(target, bufferName, packetPath, submit) {
    const load = runCommand("tmux", ["load-buffer", "-b", bufferName, packetPath]);
    if (!load.ok)
        throw new ContractError("BUFFER_LOAD_FAILED", `tmux load-buffer failed: ${load.stderr || load.stdout || "no details"}`, true);
    const paste = runCommand("tmux", ["paste-buffer", "-p", "-b", bufferName, "-t", target]);
    runCommand("tmux", ["delete-buffer", "-b", bufferName]);
    if (!paste.ok)
        throw new ContractError("PASTE_FAILED", `tmux paste-buffer failed: ${paste.stderr || paste.stdout || "no details"}`, true);
...
export function tmuxCapture(target, lines) {
    const bounded = Math.max(1, Math.min(lines, 5000));
    const res = runCommand("tmux", ["capture-pane", "-t", target, "-p", "-S", `-${bounded}`]);
    if (!res.ok)
        throw new ContractError("CAPTURE_FAILED", `tmux capture-pane failed: ${res.stderr || res.stdout || "no details"}`, true);
    return res.stdout;
}
export function tmuxTerminate(sessionName, target, retirePolicy) {
    if (retirePolicy === "explicit-only") {
        throw new ContractError("EXPLICIT_RETIRE_POLICY_REQUIRED", "session binding is explicit-only; pass a destructive --retire-policy to terminate", true);
    }
    let args;
    if (retirePolicy === "kill-pane")
    ...
    if (!res.ok)
        throw new ContractError("SESSION_TERMINATE_FAILED", `tmux ${args[0]} failed: ${res.stderr || res.stdout || "no details"}`, true);
    return res;
}
