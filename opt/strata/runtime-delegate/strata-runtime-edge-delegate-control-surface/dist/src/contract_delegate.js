...
import { assertInsidePath, ensureDir, exists, fileStamp, iso, layoutFor, readText, safePart, sha256Text, writeJson, writeText, } from "./common.js";
import { launchSession } from "./runtime.js";
...
function rel(ctx, candidate) {
    return path.isAbsolute(candidate) ? path.relative(ctx.workspaceRoot, candidate) || "." : candidate;
}
function resolveInWorkspace(ctx, candidate) {
    return path.isAbsolute(candidate) ? candidate : path.resolve(ctx.workspaceRoot, candidate);
}
function returnDirRel(assignmentId, sessionId) {
    return `.strata/returns/${safePart(assignmentId)}/${safePart(sessionId)}`;
}
function returnDirAbs(ctx, assignmentId, sessionId) {
    const returnsRoot = path.resolve(ctx.workspaceRoot, ".strata", "returns");
    const resolved = path.resolve(ctx.workspaceRoot, returnDirRel(assignmentId, sessionId));
    assertInsidePath(returnsRoot, resolved, "return directory");
    return resolved;
}
function runtimeEvidenceDir(ctx, operation, sessionId) {
...
function resultToRegister(record) {
    return {
        ok: true,
...
export function registerSession(ctx, input) {
    ...
    ensureDir(returnDirAbs(ctx, input.assignmentId, input.sessionId));
    ...
    const record = {
        ...
        retire_policy: input.retirePolicy ?? "explicit-only",
...
export function createSession(ctx, input) {
    const launchInput = {
        role: input.role,
    ...
    if (!launched.ok) {
        throw new ContractError("SESSION_CREATE_FAILED", `tmux session create failed: ${launched.tmux.stderr || launched.tmux.stdout || "unknown"}`, true, launched.evidence_dir);
    }
    const registered = registerSession(ctx, {
        ...
        runtime: input.runtime ?? "tmux_codex_cli",
        retirePolicy: input.retirePolicy ?? "explicit-only",
    });
    return {
        ...registered,
...
export function deliverDispatchPacket(ctx, input) {
...
export function dropReturnFiles(ctx, input) {
    if (!input.files.length)
        throw new ContractError("NO_RETURN_FILES", "return-drop requires at least one --file", true);
    const rdRel = returnDirRel(input.assignmentId, input.sessionId);
    const rdAbs = ensureDir(returnDirAbs(ctx, input.assignmentId, input.sessionId));
    const copied = [];
    for (const file of input.files) {
        ...
        if (!exists(sourceAbs))
            throw new ContractError("SOURCE_NOT_FOUND", `return source not found: ${sourceAbs}`, true);
...
export function reportReturnDir(ctx, input) {
    return {
        ok: true,
        runtime: "tmux_codex_cli",
...
export function captureRegisteredSession(ctx, input) {
    const binding = readBinding(ctx, input.sessionId);
...
export function terminateRegisteredSession(ctx, input) {
    const binding = readBinding(ctx, input.sessionId);
    const policy = input.retirePolicy ?? binding.retire_policy ?? "explicit-only";
...
export function listRegisteredSessions(ctx) {
    return {
        ok: true,
        runtime: "tmux_codex_cli",
