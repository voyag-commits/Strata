...
export function sessionNameFor(role, assignmentId, sessionId) {
...
export function launchSession(ctx, input) {
    ...
    if (existsResult.ok && !input.replace && !resolvedExisting)
        throw new Error(`tmux session already exists: ${sessionName}. Use --replace or --resolve-existing.`);
    let tmux;
    let terminalTab;
    if (resolvedExisting) {
        tmux = existsResult;
    ...
    const result = { contract_id: "strata.runtime_edge.launch_result.v1", ok: tmux.ok, session_name: sessionName, session_id: sessionId, record_path: recPath, evidence_dir: evidenceDir, tmux, terminal_tab: terminalTab, record };
