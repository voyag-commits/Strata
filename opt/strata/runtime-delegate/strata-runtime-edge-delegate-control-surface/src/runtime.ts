...
export function sessionNameFor(role: string, assignmentId: string, sessionId?: string | null): string {
  const base = `STRATA-${safePart(role).toUpperCase()}-${safePart(assignmentId).toUpperCase()}`;
  return sessionId ? `${base}-${safePart(sessionId).toUpperCase()}` : base;
}
...
export function launchSession(ctx: RuntimeContext, input: LaunchSessionInput): LaunchSessionResult {
  ...
  const sessionName = input.sessionName?.trim() || sessionNameFor(input.role, input.assignmentId, input.sessionId);
  const sessionId = input.sessionId?.trim() || input.assignmentId;
  ...
  const resolvedExisting = existsResult.ok && input.resolveExisting === true && !input.replace;
  if (existsResult.ok && !input.replace && !resolvedExisting) throw new Error(`tmux session already exists: ${sessionName}. Use --replace or --resolve-existing.`);
  let tmux: CommandResult;
  let terminalTab: SpawnTerminalResult | null;
  if (resolvedExisting) {
    tmux = existsResult;
    terminalTab = null;
  } else {
    ...
    tmux = runCommand("tmux", tmuxArgs, { cwd: resolved.cwd ?? ctx.workspaceRoot, encoding: "utf8" });
    if (tmux.ok) applyTmuxCosmetics(sessionName);
    terminalTab = tmux.ok ? spawnTerminalTab(sessionName) : null;
  }
  const record: RuntimeSessionRecord = {
    ...
    status: tmux.ok ? (resolvedExisting ? "observed_alive" : "created") : "launch_failed",
    launch_evidence_dir: evidenceDir,
  };
  const recPath = writeJson(recordPath(ctx, sessionName), record);
  const result: LaunchSessionResult = { contract_id: "strata.runtime_edge.launch_result.v1", ok: tmux.ok, session_name: sessionName, session_id: sessionId, record_path: recPath, evidence_dir: evidenceDir, tmux, terminal_tab: terminalTab, record };
...
export function listSessionRecords(ctx: RuntimeContext): RuntimeSessionRecord[] {
  ...
  return fs.readdirSync(layout.sessions)
    ...
    .filter((r: RuntimeSessionRecord | null): r is RuntimeSessionRecord => 
      r !== null && (r as any).contract_id === 'strata.runtime_edge.session_record.v1' && typeof (r as any).session_name === 'string'
    )
    .sort((a: RuntimeSessionRecord, b: RuntimeSessionRecord) => a.session_name.localeCompare(b.session_name));
}
