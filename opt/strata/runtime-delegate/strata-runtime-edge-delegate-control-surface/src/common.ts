...
export function safePart(value: string): string {
  ...
  if (!raw) throw new ContractError("INVALID_ARGUMENT", "safe part cannot be empty", true);
  const cleaned = raw.replace(/[^A-Za-z0-9_.-]+/g, "-").replace(/^-+|-+$/g, "");
  if (!cleaned) throw new ContractError("INVALID_ARGUMENT", "safe part cannot become empty after normalization", true);
  if (cleaned === "." || cleaned === ".." || /^\.+$/.test(cleaned)) throw new ContractError("INVALID_ARGUMENT", "safe part cannot be a dot-only path segment", true);
  return cleaned;
}
...
export function assertInsidePath(parent: string, child: string, label = "path"): void {
  if (!isInsidePath(parent, child)) throw new ContractError("INVALID_ARGUMENT", `${label} escaped expected parent: ${child}`, true);
}
