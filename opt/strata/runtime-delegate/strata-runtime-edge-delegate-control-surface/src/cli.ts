...
function retirePolicy(flags: Record<string, string | boolean | string[]>, fallback?: RetirePolicy): RetirePolicy | undefined {
  ...
  if (!value) return undefined;
  if (value === "kill-session" || value === "kill-pane" || value === "send-exit-then-kill" || value === "explicit-only") return value;
...
async function main(): Promise<void> {
  try {
    ...
    if (a === "session" && b === "name") return emit({ session_name: sessionNameFor(req(parsed.flags, "role"), req(parsed.flags, "assignment-id"), opt(parsed.flags, "session-id")) });
