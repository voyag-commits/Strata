...
import {
  captureRegisteredSession,
  createSession,
  deliverDispatchPacket,
  dropReturnFiles,
  listRegisteredSessions,
  registerSession,
  reportReturnDir,
  terminateRegisteredSession,
} from "./contract_delegate.js";
import { ContractError, failureResult, type RetirePolicy } from "./contract_shapes.js";

interface ParsedArgs { positionals: string[]; flags: Record<string, string | boolean | string[]> }

function setFlag(flags: Record<string, string | boolean | string[]>, key: string, value: string | boolean): void {
  const prior = flags[key];
  if (prior === undefined) flags[key] = value;
  else if (Array.isArray(prior)) prior.push(String(value));
  else flags[key] = [String(prior), String(value)];
}
...
function parse(argv: string[]): ParsedArgs {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    const next = argv[i + 1];
    if (!next || next.startsWith("--")) setFlag(flags, key, true);
    else { setFlag(flags, key, next); i += 1; }
  }
  return { positionals, flags };
}

function opt(flags: Record<string, string | boolean | string[]>, key: string): string | undefined {
  const value = flags[key];
  if (Array.isArray(value)) return value.length ? value[value.length - 1] : undefined;
  return typeof value === "string" && value.trim() ? value : undefined;
}
function opts(flags: Record<string, string | boolean | string[]>, key: string): string[] {
  const value = flags[key];
  if (Array.isArray(value)) return value.filter((x) => x.trim());
  if (typeof value === "string" && value.trim()) return [value];
  return [];
}
function req(flags: Record<string, string | boolean | string[]>, key: string): string {
  const value = opt(flags, key);
  if (!value) throw new ContractError("MISSING_ARGUMENT", `Missing --${key}.`, true);
  return value;
}
function bool(flags: Record<string, string | boolean | string[]>, key: string): boolean {
  const value = flags[key];
  return value === true || value === "true" || (Array.isArray(value) && value.includes("true"));
}
function num(flags: Record<string, string | boolean | string[]>, key: string): number | undefined {
  const value = opt(flags, key);
  if (!value) return undefined;
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) throw new ContractError("INVALID_ARGUMENT", `--${key} must be a number`, true);
  return parsed;
}
function splitExtra(flags: Record<string, string | boolean | string[]>): string[] {
  const value = opt(flags, "extra-args");
  if (!value) return [];
  return value.split(" ").filter(Boolean);
}
function retirePolicy(flags: Record<string, string | boolean | string[]>, fallback?: RetirePolicy): RetirePolicy | undefined {
  const value = opt(flags, "retire-policy") ?? fallback;
  if (!value) return undefined;
  if (value === "kill-session" || value === "kill-pane" || value === "send-exit-then-kill") return value;
  throw new ContractError("INVALID_ARGUMENT", "--retire-policy must be kill-session, kill-pane, or send-exit-then-kill", true);
}
...
function help(): string {
  return [
    ...
    "",
    "ADR_06_18 contract path:",
    "  strata-runtime-edge delegate session-register --assignment-id A001 --cycle-id CYCLE_001 --role \"Delegated Coordinator\" --session-id delegated_coordinator_001 --tmux-target STRATA-DESKTOP-0625-104113:0.0 [--workspace path] [--retire-policy kill-session]",
    "  strata-runtime-edge delegate session-create --assignment-id A001 --cycle-id CYCLE_001 --role \"Change Author\" --session-id change_author_001 [--config path] [--replace] [--extra-args \"...\"]",
    ...
    "  strata-runtime-edge delegate session-terminate --session-id delegated_coordinator_001 [--retire-policy kill-session] [--workspace path]",
...
async function main(): Promise<void> {
  try {
    ...
    if (a === "delegate" && b === "session-register") return emit(registerSession(ctx, {
    ...
    if (a === "delegate" && b === "session-create") return emit(createSession(ctx, {
      assignmentId: req(parsed.flags, "assignment-id"),
      cycleId: opt(parsed.flags, "cycle-id") ?? null,
      role: req(parsed.flags, "role"),
      sessionId: req(parsed.flags, "session-id"),
    ...
    if (a === "delegate" && b === "dispatch-deliver") return emit(deliverDispatchPacket(ctx, {
      sessionId: req(parsed.flags, "session-id"),
    ...
    if (a === "delegate" && b === "return-drop") return emit(dropReturnFiles(ctx, {
      assignmentId: req(parsed.flags, "assignment-id"),
      sessionId: req(parsed.flags, "session-id"),
      files: opts(parsed.flags, "file").length ? opts(parsed.flags, "file") : [req(parsed.flags, "source")],
    }));
    if (a === "delegate" && b === "return-dir") return emit(reportReturnDir(ctx, {
      assignmentId: req(parsed.flags, "assignment-id"),
      sessionId: req(parsed.flags, "session-id"),
    }));
    if (a === "delegate" && b === "session-capture") return emit(captureRegisteredSession(ctx, {
      sessionId: req(parsed.flags, "session-id"),
      lines: num(parsed.flags, "lines") ?? 200,
    }));
    if (a === "delegate" && b === "session-terminate") return emit(terminateRegisteredSession(ctx, {
      sessionId: req(parsed.flags, "session-id"),
      retirePolicy: retirePolicy(parsed.flags),
    }));
    if (a === "delegate" && b === "session-list") return emit(listRegisteredSessions(ctx));
    ...
    if (a === "session" && b === "launch") return emit(launchSession(ctx, { role: req(parsed.flags, "role"), assignmentId: req(parsed.flags, "assignment-id"), sessionId: opt(parsed.flags, "session-id"), workerId: opt(parsed.flags, "worker-id"), sessionName: opt(parsed.flags, "session"), configPath: opt(parsed.flags, "config"), replace: bool(parsed.flags, "replace"), extraArgs: splitExtra(parsed.flags) }));
    ...
    if (error instanceof ContractError) emit(failureResult(error));
    else emit(failureResult(new ContractError("INTERNAL_ERROR", error instanceof Error ? error.message : String(error), false)));
  }
}
