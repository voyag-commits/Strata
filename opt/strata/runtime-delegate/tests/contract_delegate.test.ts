...
import { injectNotice, renderBoundedNotice, type RuntimeEdgeNotice } from "../src/dispatch_edge.js";
...
function installFakeTmux(root: string): { bin: string; pastePath: string; logPath: string } {
...
function sampleNotice(): RuntimeEdgeNotice {
  return {
...
test("dispatch-deliver resolves session binding and pastes exact packet", () => {
  ...
  assert.equal(readJson<Record<string, unknown>>(dispatchLog).packet_sha256, expectedSha);
});
...
test("session-capture and session-terminate use registered tmux identity", () => {
  const root = tempRoot();
  const fake = installFakeTmux(root);
  const ctx = createRuntimeContext(process.cwd(), root);
  ...
  const terminated = terminateRegisteredSession(ctx, { sessionId: "delegated_coordinator_001" });
...
test("missing packet failure is machine-readable through ContractError", () => {
  ...
  const failure = failureResult(caught as ContractError);
